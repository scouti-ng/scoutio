...
module.exports = (router, rnio) => {
    ...
    router.get('/client', {
        func: (params, client) => {
            params.styles = ['/game/stoptime/common.css', '/game/stoptime/client.css'];
            params.scripts = ['/game.js', '/game/stoptime/client.js'];
            return files.makePage(files.game.stoptime.client, client, params);
        }
    });

    router.get('/server', {
        func: (params, client) => {
            params.styles = ['/game/stoptime/common.css', '/game/stoptime/server.css'];
            params.scripts = ['/game.js', '/game/stoptime/server.js'];
            // return client.token;
            return files.makePage(files.game.stoptime.server, client, params);
        }
    });
    ...
    router.ws('/register', {
        ...
        func: async(params, client) => {
            ...
            // If a client registers, add it to the stoptime players and broadcast the joining.
    ...
    router.on('wsClose', (params, client) => {
        if (!client.token.room || client.token.game != 'stoptime') return;
        let roomInfo = GameUtils.getRoom(client.token.room);
        if (client.token.type == 'client') {
            let playerInfo = roomInfo.players[client.token.username];
